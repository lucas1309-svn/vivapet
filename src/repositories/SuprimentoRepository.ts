...
export class SuprimentoRepository{
    ...
    // METODO PARA SALVAR UM SUPRIMENTO NA BASE DE DADOS
...
// METODO PARA ATUALIZAR UM SUPRIMENTO NA BASE DE DADOS
public async atualizar(suprimento: Suprimento){

        const query = `UPTADE suprimento SET
       nome = $2, unidade = $3, quantidade_estoque = $4, quantidade_minima = $5, ativo = $6 WHERE suprimento_id = $1`;


        const values = [ suprimento.id_suprimento, suprimento.nome, suprimento.unidade, suprimento.quantidade_estoque, suprimento.quantidade_minima, suprimento.data_cadastro, suprimento.ativo]
